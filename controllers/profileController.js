...
const follow = async (req, res) => {
  ...
  try {
    ...
    if (
      followed.followers.filter((follower) => follower.username === followerId)
        .length === 1
    ) {
      //Si ya lo sigue, debe dejar de seguirlo
      const followerList = followed.followers.filter(
        (follower) => follower.username !== followerId
      );
      const followedList = follower.following.filter(
        (followed) => followed.username !== followedId
      );
